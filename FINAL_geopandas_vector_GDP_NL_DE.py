...
df = pd.read_csv(r'C:\Users\wald_\OneDrive\Dokumente\Hochschule der Medien\International_Projekt_Rituals_HvA_HdM\FINAL_NL_DE_rituals_locations_with_kpis.csv', delimiter=';', encoding='ISO-8859-1')
...
# 3. Load the Eurostat data (GDP structure indicators by NUTS 3 region)
eurostat_data = pd.read_csv(r'C:\Users\wald_\OneDrive\Dokumente\Hochschule der Medien\International_Projekt_Rituals_HvA_HdM\FINAL_nut3_gdp_2019_NL_DE.csv', delimiter=',', encoding='ISO-8859-1')
...
folium.Choropleth(
    ...
    fill_opacity=0.7,
    line_opacity=0.2,
    legend_name='GDP Structure NL&DE (2019)',
...
for idx, row in merged_gdf.iterrows():
    folium.GeoJson(
        row['geometry'],
        tooltip=folium.Tooltip(f"Region: {row['NUTS_ID']}<br>GDP: {row['2019']}"),
        popup=folium.Popup(f"Region: {row['NUTS_ID']}<br>GDP: {row['2019']}"),
...
m.save("FINAL_rituals_avenda_store_map_with_vector_tiles_gdp_NL_DE_MAP.html")
